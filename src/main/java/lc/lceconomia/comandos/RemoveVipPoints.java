...
public class RemoveVipPoints extends SubComando{
    ...
    public String getSyntax() {
        return "/vippoints remove <jugador> <cantidad>";
    }

    @Override
    public String getDescription() {
        return "Quitar VipPoints.";
    }
    ...
    public void perform(Player p, String[] args) {
        if(args.length > 2){
            ...
            vp.subtractVipPoints(amount);
            t.sendMessage(ChatColor.GREEN + "Se han removido " + ChatColor.GRAY + amount + ChatColor.GREEN +" VipPoints a tu cuenta.");
            p.sendMessage(ChatColor.GREEN + "Se han removido " + ChatColor.GRAY + amount + ChatColor.GREEN +" VipPoints a la cuenta de "+ChatColor.YELLOW+t.getName()+""+ChatColor.GREEN+".");
        }else{
